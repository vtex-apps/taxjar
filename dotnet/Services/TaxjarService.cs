    ...
    public class TaxjarService : ITaxjarService
    {
        ...
        public async Task<string> SendRequest(string endpoint, string jsonSerializedData, HttpMethod httpMethod)
        {
            ...
            try
            {
                ...
                if(!response.IsSuccessStatusCode)
                {
                    try
                    {
                        _context.Vtex.Logger.Warn("SendRequest", null, $"[{httpMethod}] '{jsonSerializedData}'\rto '{endpoint}'\rResponse [{response.StatusCode}] '{responseContent}'");
                        ErrorResponse errorResponse = JsonConvert.DeserializeObject<ErrorResponse>(responseContent);
                        responseContent = $"{TaxjarConstants.ERROR_INDICATOR}:{errorResponse.Detail}";
                    }
                    catch (Exception ex)
                    {
                        _context.Vtex.Logger.Error("SendRequest", null, $"Could not parse error response '{responseContent}'", ex);
                    }
                }
            }
            catch (Exception ex)
        ...
        public async Task<OrderResponse> CreateOrder(CreateTaxjarOrder taxjarOrder)
        {
            ...
            if (!string.IsNullOrEmpty(response))
            {
                ...
                {
                    if(response.Contains("already imported for your user account"))
                    {
